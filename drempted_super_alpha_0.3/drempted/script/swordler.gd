...
func _physics_process(delta):
	...
	if Input.is_action_just_pressed("jump") and onground == 0 and justflipped == 0 and wallslide == 0 and flipping == 0 and pounding == 0 and airdashed == 0 and not walljumpbuffer > 0 and mov_axis.y <= 0:
		velocity.y = clamp(velocity.y, -1000, 0)
		velocity += mov_axis.normalized() * 100
		velocity.y -= 100
		airdashed = 1
		if mov_axis == Vector2.ZERO:
			velocity.y -= 100
